  ...
  return (
    <AccountantLayout>
      ...
      <div className="flex flex-col items-center py-8 mb-4">
        <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-violet-400 to-violet-600 flex items-center justify-center text-white text-3xl font-bold shadow-lg shadow-violet-200 mb-3">
          {user?.name?.[0]?.toUpperCase()}
        </div>
        <h1 className="text-xl font-bold text-gray-900">{user?.name}</h1>
        <p className="text-sm text-gray-400">{user?.email}</p>
        <span className="mt-2 badge bg-violet-100 text-violet-700">Accountant</span>
      </div>
